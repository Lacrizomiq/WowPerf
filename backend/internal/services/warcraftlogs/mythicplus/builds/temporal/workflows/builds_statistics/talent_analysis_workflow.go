...
// TalentAnalysisWorkflow implements the talent analysis workflow
type TalentAnalysisWorkflow struct{}

// NewTalentAnalysisWorkflow creates a new instance of the talent analysis workflow
func NewTalentAnalysisWorkflow() definitions.TalentAnalysisWorkflow {
	return &TalentAnalysisWorkflow{}
}

// Execute runs the talent analysis workflow
func (w *TalentAnalysisWorkflow) Execute(ctx workflow.Context, params models.TalentAnalysisWorkflowParams) (*models.TalentAnalysisWorkflowResult, error) {
	logger := workflow.GetLogger(ctx)
	logger.Info("Starting talent analysis workflow",
	...
	workflowStateID := fmt.Sprintf("talent-analysis-%s", workflowID)
	...
	for _, spec := range params.Spec {
		...
		for _, dungeon := range params.Dungeon {
			...
			logger.Info("Processing talent analysis",
				"class", spec.ClassName,
				"spec", spec.SpecName,
				"dungeon", dungeon.Name)

			// Execute the talent analysis activity
			var activityResult models.TalentAnalysisWorkflowResult
			...
			if err != nil {
				if common.IsRateLimitError(err) {
					...
					logger.Info("Rate limit reached during talent analysis",
						"class", spec.ClassName,
						"spec", spec.SpecName,
						"dungeon", dungeon.Name)
				...
				logger.Error("Failed to process talent analysis",
					"class", spec.ClassName,
					"spec", spec.SpecName,
					"dungeon", dungeon.Name,
					"error", err)
			...
			totalTalents += activityResult.TalentsAnalyzed

			// Update the workflow state with progress
			workflowState := &warcraftlogsBuilds.WorkflowState{
				ID:             workflowStateID,
				ItemsProcessed: int(totalTalents),
				UpdatedAt:      workflow.Now(ctx),
			}
			_ = workflow.ExecuteActivity(stateCtx, definitions.UpdateWorkflowStateActivity, workflowState).Get(ctx, nil)

			logger.Info("Successfully processed talent analysis",
				"class", spec.ClassName,
				"spec", spec.SpecName,
				"dungeon", dungeon.Name,
				"buildsProcessed", activityResult.TotalBuilds,
				"talentsProcessed", activityResult.TalentsAnalyzed)
	...
	result.TalentsAnalyzed = totalTalents
	...
	logger.Info("Talent analysis workflow completed",
		"totalBuilds", totalBuilds,
		"talentsAnalyzed", totalTalents,

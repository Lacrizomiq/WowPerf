 ...
 * 
 * This migration fixes column ambiguity issues in the analytical functions created in migration 027.
 * The main changes are:
 * 1. Qualified all column references with table aliases
 * 2. Fixed data type conversion issues in function results
 * 3. Added appropriate aliases in subqueries to avoid ambiguity
 */

-- Function 1: Get Popular Items By Slot (Fixed)
...
-- Function 2: Get Enchant Usage (Fixed)
...
BEGIN
    ...
    SELECT 
        ...
        bs.permanent_enchant_name,
        COUNT(*) as usage_count,
        AVG(bs.avg_keystone_level) as avg_keystone_level,
        AVG(bs.avg_item_level) as avg_item_level,
        MAX(bs.max_keystone_level) as max_keystone_level,
        ROW_NUMBER() OVER (PARTITION BY bs.item_slot ORDER BY COUNT(*) DESC) as rank
    FROM build_statistics bs
    WHERE bs.class = p_class
    AND bs.spec = p_spec
    AND bs.has_permanent_enchant = true
    GROUP BY bs.item_slot, bs.permanent_enchant_id, bs.permanent_enchant_name
    ORDER BY bs.item_slot, rank;
END;
$$ LANGUAGE plpgsql;

-- Function 3: Get Gem Usage (Fixed)
...
BEGIN
    ...
    SELECT 
        bs.item_slot,
        bs.gems_count,
        bs.gem_ids as gem_ids_array,
        bs.gem_icons as gem_icons_array,
        bs.gem_levels as gem_levels_array,
        COUNT(*) as usage_count,
        AVG(bs.avg_keystone_level) as avg_keystone_level,
        AVG(bs.avg_item_level) as avg_item_level,
        ROW_NUMBER() OVER (PARTITION BY bs.item_slot ORDER BY COUNT(*) DESC) as rank
...
-- Function 4: Get Top Talent Builds (Fixed)
...
-- Function 5: Get Talent Builds By Dungeon (Fixed)
...
-- Function 6: Get Stat Priorities (Fixed)
...
BEGIN
    ...
    WITH stat_aggregates AS (
        SELECT 
            ...
            SUM(ss.sample_size) as total_samples,
    ...
    SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY sa.stat_category ORDER BY sa.avg_value DESC) as priority_rank
    FROM stat_aggregates sa
    ORDER BY sa.stat_category, priority_rank;
END;
$$ LANGUAGE plpgsql;

-- Function 7: Get Optimal Build (Fixed)
...
BEGIN
    ...
    top_stats AS (
        SELECT STRING_AGG(s.stat_name, ' > ' ORDER BY s.avg_value DESC) as stat_priority
        FROM (
            SELECT 
...
-- Function 8: Get Spec Comparison (Fixed)
...
BEGIN
    ...
    WITH spec_stats AS (
        SELECT 
            ts.spec,
            AVG(ts.avg_keystone_level) as avg_keystone_level,
            MAX(ts.max_keystone_level) as max_keystone_level,
            AVG(ts.avg_item_level) as avg_item_level,
            COUNT(DISTINCT ts.encounter_id) as dungeons_count
        FROM talent_statistics ts
        WHERE ts.class = p_class
        GROUP BY ts.spec
    ),
    spec_stat_priorities AS (
        SELECT 
            sv.spec,
            STRING_AGG(sv.stat_name, ' > ' ORDER BY sv.avg_stat_val DESC) as stat_priority
        FROM (
            SELECT 
                ss.spec, 
                ss.stat_name,
...
-- Bonus Function: Get Class Spec Summary (Fixed)

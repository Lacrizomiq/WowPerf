...
import { useGetGlobalPopularItems } from "@/hooks/useBuildsAnalysisApi";
import {
  WowClassParam,
  WowSpecParam,
...
interface AllDungeonsGearProps {
  className: WowClassParam;
  spec: WowSpecParam;
  selectedSlotId: number | null;
}

export default function AllDungeonsGear({
  className,
  spec,
  selectedSlotId,
}: AllDungeonsGearProps) {
  // Hooks pour la navigation
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  // Get global popular items data
  const {
    data: itemsData,
    isLoading,
    error,
  } = useGetGlobalPopularItems(className, spec);
  ...
  const title =
    selectedSlotId !== null
      ? `Most Popular ${ITEM_SLOT_NAMES[selectedSlotId]} Across All Dungeons`
      : "Most Popular Gear Across All Dungeons";
  ...
  const description =
    selectedSlotId !== null
      ? `These items represent the most frequently used ${ITEM_SLOT_NAMES[
       ...
       across all Mythic+ dungeons for ${classNameToPascalCase(
         className
       )} ${specNameToPascalCase(spec)}.`
      : `These items represent the most frequently used gear across all Mythic+ dungeons for 
